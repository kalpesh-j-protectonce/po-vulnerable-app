...
module.exports = function(app, db) {
  ...
  app.get('/api/user/login', (req, res) => {
    if(req.query.name && req.query.pwd ) {
      if(req.query.pwd !== "1234") {
        req.protectonce.reportLogin(false, req.query.name );
      } else {
        req.protectonce.reportLogin(true, req.query.name );
        //req.protectonce.reportAuth(req.query.name );
      }
    }
